...
const ScoreBadgeWithIcon = ({ score }: { score: number }) => {
  return (
    <div
      className={cn(
        "flex flex-row gap-1 items-center px-2 py-0.5 rounded-[96px]",
        score > SCORE_VALUE.high ? "bg-badge-green" : score > SCORE_VALUE.mid ? "bg-badge-yellow" : "bg-badge-red"
      )}
    >
      <img src={score > SCORE_VALUE.high ? "/icons/check.svg" : "/icons/warning.svg"} alt="score" className="size-4" />
      <p
        className={cn(
          "text-sm font-medium",
          score > SCORE_VALUE.high
            ? "text-badge-green-text"
            : score > SCORE_VALUE.mid
...
const CategoryHeader = ({ title, categoryScore }: { title: string; categoryScore: number }) => {
  return (
    <div className="flex flex-row gap-4 items-center py-2">
      ...
      <ScoreBadgeWithIcon score={categoryScore} />
    </div>
  );
};
